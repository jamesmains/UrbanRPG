...
namespace SnapshotShaders.BuiltIn {
    ...
    public sealed class Halftone : PostProcessEffectSettings {
        ...
        public TextureParameter halftoneTexture = new();

        [Tooltip("How soft the transition between light and dark is.")]
        public FloatParameter softness = new() {value = 0.5f};

        [Tooltip("Size of the halftone 'dots' on the screen.")]
        public FloatParameter textureSize = new() {value = 4.0f};

        [Tooltip(
            "Use this vector to remap the minimum and maximum luminance values used in calculations. Default is (0, 1).")]
        public Vector2Parameter minMaxLuminance = new() {value = new Vector2(0.0f, 1.0f)};
