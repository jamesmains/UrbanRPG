...
namespace SnapshotShaders.BuiltIn {
    ...
    public class Neon : PostProcessEffectSettings {
        ...
        public FloatParameter saturationFloor = new() {value = 1.0f};

        [Range(0.0f, 1.0f)] [Tooltip("Lightness/value values lower than this will be clamped to this.")]
        public FloatParameter lightnessFloor = new() {value = 1.0f};

        [Range(0.0f, 1.0f)] [Tooltip("Threshold for colour-based edge detection.")]
        public FloatParameter colorSensitivity = new() {value = 0.1f};

        [Range(0.0f, 1.0f)] [Tooltip("Strength of colour-based edges.")]
        public FloatParameter colorStrength = new() {value = 1.0f};

        [Range(0.0f, 1.0f)] [Tooltip("Threshold for depth-based edge detection.")]
        public FloatParameter depthSensitivity = new() {value = 0.01f};

        [Range(0.0f, 1.0f)] [Tooltip("Strength of depth-based edges.")]
        public FloatParameter depthStrength = new() {value = 1.0f};

        [Range(0.0f, 1.0f)] [Tooltip("Threshold for normal-based edge detection.")]
        public FloatParameter normalSensitivity = new() {value = 0.1f};

        [Range(0.0f, 1.0f)] [Tooltip("Strength of normal-based edges.")]
        public FloatParameter normalStrength = new() {value = 1.0f};

        [Range(0.0f, 1.0f)] [Tooltip("Pixels past this depth threshold will not be edge-detected.")]
        public FloatParameter depthThreshold = new() {value = 0.99f};

        [Tooltip("Color of non-neon pixels.")] public ColorParameter backgroundColor = new() {value = Color.black};

        [ColorUsage(true, true)] [Tooltip("Extra color multiplier (allows for emissive HDR colors).")]
        public ColorParameter emissiveEdgeColor = new() {value = Color.white};
