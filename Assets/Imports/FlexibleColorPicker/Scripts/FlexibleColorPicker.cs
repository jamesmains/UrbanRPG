...
public class FlexibleColorPicker : MonoBehaviour {
    ...
    [Tooltip("Connection to the FCP's hexadecimal input field.")] [SerializeField]
#if TMPro
    ...
    [Tooltip("Connection to the FCP's mode dropdown menu.")] [SerializeField]
#if TMPro
    ...
    [Tooltip(
        "The (starting) 2D picking mode, i.e. the 2 color values that can be picked with the large square picker.")]
    [SerializeField]
    private MainPickingMode mode;

    [Tooltip("Sprites to be used in static mode on the main picker, one for each 2D mode.")] [SerializeField]
    ...
    //public settings
    [Tooltip(
        "Color set to the color picker on Start(). If you wish to set a starting color via script please used the standard color parameter of the FCP in stead.")]
    [SerializeField]
    ...
    //advanced settings
    [Tooltip(
        "More specific settings for color picker. Changes are not applied immediately, but require an FCP update to trigger.")]
    ...
    public class AdvancedSettings {
        ...
        public class PSettings {
            ...
            public Vector2 range = new(0f, 1f);
    ...
    /// </summary>
    /// <param name="type">
    ///     Select component of the color to change, 1: red, 2: green, 3:blue, 4:hue, 5:saturation, 6:value,
    ///     7:alpha
    /// </param>
    /// <param name="delta">
    ///     How much should the color be changed, note this value is normalized and the result is clamped,
    ///     including for hue.
    /// </param>
    public void ShiftColor(int type, float delta) {
        var pt = (PickerType) type;
        var value = GetValue1D(pt) + delta;
        bufferedColor = PickColor1D(bufferedColor, pt, value);
    ...
    public void ShiftHue(float delta) {
        var pt = PickerType.H;
        var h = Mathf.Repeat(GetValue1D(pt) + delta, 6f) / 6f;
        bufferedColor = PickColor1D(bufferedColor, pt, h);
