        ...
        Pass {
            CGPROGRAM
                ...
                fixed4 frag (v2f i) : COLOR {
                    ...
                    if (falloff > 0) {
                        float2 mid0 = edgePosition;
                        mid0.x += leftEdgeLocal.x;
                        mid0.y += leftEdgeLocal.y;

                        float2 mid1 = edgePosition;
                        mid1.x += rightEdgeLocal.x;
                        mid1.y += rightEdgeLocal.y;

                        float2 p = PointToLine(pixelPos, mid1, mid0);

                        d = distance(p, pixelPos) * (10 - falloff);
                    }
