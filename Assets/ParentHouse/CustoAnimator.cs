...
namespace ParentHouse {
    public class CustoAnimator : MonoBehaviour {
        ...
        [SerializeField] private List<InputToDirectionDefiner> inputAdjustments = new();
        [SerializeField] private List<SpriteSheetOffsetDefiner> sheetOffsetAdjustments = new();
        ...
        public void UpdateActor(Actor incomingActor) {
            ...
            foreach (var slot in gearSlots) {
                slot.gear = null;
                foreach (var gearOption in actor.EquippedGear.Where(gearOption =>
                             slot.gearType == gearOption.type && gearOption.gear != null)) {
        ...
        public void SyncFrames() {
            var i = from gearSlot in gearSlots
                orderby gearSlot.frameIndex descending
                select gearSlot;
            var index = i.First().frameIndex;
            foreach (var gearSlot in gearSlots) gearSlot.frameIndex = index;
        }
        ...
        public void ChangeDirection(Vector2 rawDirection, int action) {
            ...
            foreach (var inputMirror in inputAdjustments) {
                var targetDirection = new Vector2(inputMirror.xRequirement, inputMirror.yRequirement);
            ...
            if (newDirection != -1 && currentDirection != newDirection) {
        ...
        public void SetAction(int a) {
            ...
            actionIndex = a;
            foreach (var slot in gearSlots.Where(slot => slot.gameObject.activeSelf)) slot.LoadAnimation(actionIndex);
        }
    }
